...
\begin{itemize}
 \item \textbf{Movimentação:} Nas operações de movimentação, os agentes são movimentados dentro de um ambiente virtual com topologia matricial através de suas vizinhanças de Moore às posições escolhidas aleatoriamente, respeitando os limites do ambiente.
...
Na modelagem realizada, as especificações às quantidades são $Pl = 0$, $Px = 6$, $Py = 15$, $Pc = 24$, $Pe = 30$, $C\#l = 26$, $C\#x = 23$, $C\#y = 23$, $C\#c = 26$ e $C\#e = 30$, de modo que \ref{eq:bitstring_get} é então reescrita para o ciclo $t$ designado como \textit{old} e \textit{new}, respectivamente, como em \ref{eq:bitstring_get_2}.
\begin{equation}
  \begin{cases}
...
\begin{equation}
  \begin{cases}
    \ll_{circ} = \big(M \ll_{arth}(shift)\big) \, | \, \big(M \gg_{arth}(N\_BITS - shift)\big) \\
    \gg_{circ} = \big(M \gg_{arth}(shift)\big) \, | \, \big(M \gg_{arth}(N\_BITS - shift)\big) \\
  \end{cases}
  \label{eq:bitstring_circ}
\end{equation}

Em \ref{eq:bitstring_circ}, $M$ denota a palavra computacional em que está sendo aplicada a operação de deslocamento circular, $N\_BITS$ denota a quantidade de bits totais, \textit{shift} denota a quantidade de \textit{bits} para os deslocamentos e $|$ denota a operação lógica OR, "ou inclusivo", que é realizada \textit{bit} a \textit{bit} à palavra.
...
\begin{equation}
  \begin{cases}
    \ll_{circ} = \big(M \ll_{arth}(shift)\big) \, | \, \big(M \gg_{arth}(32 - shift)\big) \\
    \gg_{circ} = \big(M \gg_{arth}(shift)\big) \, | \, \big(M \gg_{arth}(32 - shift)\big) \\
  \end{cases}
  \label{eq:bitstring_circ_2}
\end{equation}

Subsequentemente são utilizadas as operações definidas em \ref{eq:bitstring_circ_2}, para implementar o operador espaço-temporal $\lambda\big(\chi(t)\big)$ que movimenta o agente da posição $(i,j)$ para uma posição $\big(\xi, \eta\big)$ no ciclo de tempo atual, $t$ \textit{(old)}, para o ciclo de tempo, $t+1$ \textit{(new)}. As operações definidas em \ref{bitstring_set} resultam na atualização dos atributos de identificação do lote, das coordenadas $x$ e $y$ da posição, do controle e do estado do agente.
\begin{equation}
  \scriptsize
  \begin{cases}
    \chi\big(L(t + 1), X(t), Y(t), C(t), E(t)\big) = \chi\big(L(t), X(t), Y(t), C(t), E(t)\big) \gg_{circ}(Al) \gg_{arth}(\#l) \ll_{arth} (\#l) \, | \, L(t + 1) \ll_{circ}(Al) \\
    \chi\big(L(t), X(t + 1), Y(t), C(t), E(t)\big) = \chi\big(L(t), X(t), Y(t), C(t), E(t)\big) \gg_{circ}(Ax) \gg_{arth}(\#x) \ll_{arth} (\#x) \, | \, X(t + 1) \ll_{circ}(Ax) \\
    \chi\big(L(t), X(t), Y(t + 1), C(t), E(t)\big) = \chi\big(L(t), X(t), Y(t), C(t), E(t)\big) \gg_{circ}(Ay) \gg_{arth}(\#y) \ll_{arth} (\#y) \, | \, Y(t + 1) \ll_{circ}(Ay) \\
    \chi\big(L(t), X(t), Y(t), C(t + 1), E(t)\big) = \chi\big(L(t), X(t), Y(t), C(t), E(t)\big) \gg_{circ}(Ac) \gg_{arth}(\#c) \ll_{arth} (\#c) \, | \, C(t + 1) \ll_{circ}(Ac) \\
    \chi\big(L(t), X(t), Y(t), C(t), E(t + 1)\big) = \chi\big(L(t), X(t), Y(t), C(t), E(t)\big) \gg_{circ}(Ae) \gg_{arth}(\#e) \ll_{arth} (\#e) \, | \, E(t + 1) \ll_{circ}(Ae) \\
  \end{cases}
  \label{bitstring_set}
\end{equation}

Em \ref{bitstring_set}, $Al$, $Ax$, $Ay$, $Ac$ e $Ae$ designam, respectivamente, as quantidades de \textit{bits} anteriores à cada campo. A quantidade de \textit{bits} anteriores à cada campo é a quantidade de \textit{bits} existentes que são menos significativos àqueles do campo em questão. Os termos $\#l$, $\#x$, $\#y$, $\#c$ e $\#e$ designam, respectivamente, as quantidades de \textit{bits} de cada campo.

Como $Al = 26$, $Ax = 17$, $Ay = 8$, $Ac = 2$, $Ae = 0$, $\#l = 6$, $\#x = 9$, $\#y = 9$, $\#c = 6$ e $\#e = 2$, a especificação às quantidades adotadas na modelagem, \ref{bitstring_set} é reescrita como em \ref{bitstring_set_2}.
\begin{equation}
  \scriptsize
  \begin{cases}
    \chi\big(L_{new}, X_{old}, Y_{old}, C_{old}, E_{old}\big) = \chi\big(L_{old}, X_{old}, Y_{old}, C_{old}, E_{old}\big) \gg_{circ}(26) \gg_{arth}(6) \ll_{arth}(6) \, | \, L_{new} \ll_{circ}(26) \\
  
    \chi\big(L_{old}, X_{new}, Y_{old}, C_{old}, E_{old}\big) = \chi\big(L_{old}, X_{old}, Y_{old}, C_{old}, E_{old}\big) \gg_{circ}(17) \gg_{arth}(9) \ll_{arth}(9) \, | \, X_{new} \ll_{circ}(17) \\
    
    \chi\big(L_{old}, X_{old}, Y_{new}, C_{old}, E_{old}\big) = \chi\big(L_{old}, X_{old}, Y_{old}, C_{old}, E_{old}\big) \gg_{circ}(8) \gg_{arth}(9) \ll_{arth}(9) \, | \, Y_{new} \ll_{circ}(8) \\
    
    \chi\big(L_{old}, X_{old}, Y_{old}, C_{new}, E_{old}\big) = \chi\big(L_{old}, X_{old}, Y_{old}, C_{old}, E_{old}\big) \gg_{circ}(2) \gg_{arth}(6) \ll_{arth}(6) \, | \, C_{new} \ll_{circ}(2) \\
    
    \chi\big(L_{old}, X_{old}, Y_{old}, C_{old}, E_{new}\big) = \chi\big(L_{old}, X_{old}, Y_{old}, C_{old}, E_{old}\big) \gg_{circ}(0) \gg_{arth}(2) \ll_{arth}(2) \, | \, E_{new} \ll_{circ}(0) \\
  \end{cases}
  \label{bitstring_set_2}
\end{equation}

Nas Tabelas \ref{tab:tamanhos}, \ref{tab:complementares}, \ref{tab:posteriores} e \ref{tab:anteriores} são apresentados sistematicamente os valores relativos às quantidades de \textit{bits} dos tamanhos, complementos, posteriores e anteriores de cada atributo do agente $\chi$.

\begin{table}[h!]
\begin{minipage}{.5\textwidth}
\centering
\caption{Tabela de tamanhos}
\begin{tabular}{c|c}
  \label{tab:tamanhos}
  Atributo & Valor \\ \hline
  Identificador do Lote $L$ 	& 6 \\
  Coordenada $X$ 		& 9 \\
  Coordenada $Y$ 		& 9 \\
  Contador de Controle $C$ 	& 6 \\
  Estado $E$ 			& 2 \\
\end{tabular}
\end{minipage} 
\begin{minipage}{.5\textwidth}
\centering
\caption{Tabela de complementares}
\begin{tabular}{c|c}
  \label{tab:complementares}
  Atributo & Valor \\ \hline
  Identificador do Lote $L$ 	& 26 \\
  Coordenada $X$ 		& 23 \\
  Coordenada $Y$ 		& 23 \\
  Contador de Controle $C$ 	& 26 \\
  Estado $E$ 			& 30 \\
\end{tabular}
\end{minipage}
\end{table} 
...
\begin{minipage}{.5\textwidth}
\centering
\caption{Tabela de posteriores}
\begin{tabular}{c|c}
  \label{tab:posteriores}
  Atributo & Valor \\ \hline
  Identificador do Lote $L$ 	& 0 \\
  Coordenada $X$ 		& 6 \\
  Coordenada $Y$ 		& 15 \\
  Contador de Controle $C$ 	& 24 \\
  Estado $E$ 			& 30 \\
\end{tabular}
\end{minipage} 
\begin{minipage}{.5\textwidth}
\centering
\caption{Tabela de anteriores}
\begin{tabular}{c|c}
