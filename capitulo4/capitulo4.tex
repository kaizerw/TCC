...
\begin{equation}
  E_R^{CPU, GPU} = \frac{|CPU(i) - GPU(i)|}{CPU(i)}
\label{eq:erro_relativo1}
\end{equation}


\begin{equation}
  E_R^{C, B} = \frac{|C(i) - B(i)|}{C(i)}
...
Pode-se observar ainda que existem picos para a população de indivíduos suscetíveis próximos ao ciclo $70$ em todos os resultados. Eles acontecem pois as quantidades de indivíduos suscetíveis próximo a este ciclo são muito próximas de zero, o que acarreta em um aumento de sensibilidade no cálculo do erro relativo entre as quantidades. É possível observar, por meio da análise das Figuras \ref{fig:convencional_cpu} a \ref{fig:bitstring_gpu} que, quando as quantidades de uma determinada população se aproximam de zero em um ciclo de tempo, os erros relativos entre suas implementações aumentam aproximadamente neste mesmo ciclo. 

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{Figuras/Acuracia/1-erros_relativos_CPU_CPU.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{Figuras/Acuracia/2-erros_relativos_GPU_GPU}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{Figuras/Acuracia/3-erros_relativos_CPU_GPU_C.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{Figuras/Acuracia/4-erros_relativos_CPU_GPU_B.png}
