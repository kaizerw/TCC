...
Pode-se observar ainda que existem diversos picos para as populações de indivíduos em todos os resultados. Eles acontecem pois as quantidades de indivíduos são muito próximas de zero, o que acarreta em um aumento de sensibilidade no cálculo do erro relativo entre as quantidades. É possível observar, por meio da análise das Figuras \ref{fig:convencional_cpu} a \ref{fig:bitstring_gpu} que, quando as quantidades de uma determinada população se aproximam de zero em um ciclo de tempo, os erros relativos entre suas implementações aumentam aproximadamente neste mesmo ciclo. 
