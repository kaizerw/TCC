...
Pode-se observar ainda que existem picos para a população de indivíduos suscetíveis próximos ao ciclo $70$ em todas as figuras. Estes picos acontecem pois as quantidades de indivíduos suscetíveis próximo a este ciclo são muito próximas de zero, o que acarreta em um aumento de sensibilidade no cálculo do erro relativo entre estas quantidades. É possível observar, por meio da análise das Figuras \ref{fig:convencional_cpu} a \ref{fig:acuracia_GPU_GPU_B} que, quando as quantidades de uma determinada população se aproximam de zero em um ciclo de tempo, os erros relativos entre suas implementações aumentam aproximadamente neste mesmo ciclo. 
